...
int main() {
  ...
  for (i = 0; i < NUM_PHILS; i++) pthread_join(tid, NULL);
  return 0;
}
...
int leftOf(int i) { return (i + NUM_PHILS - 1) % NUM_PHILS; }

int rightOf(int i) { return (i + 1) % NUM_PHILS; }
...
void test(int i) {
  ...
  // then let me eat.
  if (state[i] == HUNGRY && state[leftOf(i)] != EATING &&
